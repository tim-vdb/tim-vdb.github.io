...
document.addEventListener("DOMContentLoaded", function () {
    ...
    const learn_skills = document.getElementById("learn_skills");
    ...
    // skills
    if (skills_cards) {
        function skills_cardsFunction(tab_skills, skills_cards) {
        ...
        skills_cardsFunction(coding, skills_cards);
        skills_cardsFunction(design, skills_cards);
        skills_cardsFunction(CMS, skills_cards);
        skills_cardsFunction(learning_skills, learnSkills_cards);
    }
    ...
    gsap.fromTo(
        "#learnSkills_cards .cards", // Cible toutes les cartes
        {
            opacity: 0,
            y: 50 // Départ légèrement en dessous
        },
        {
            ...
            ease: "power2.out",
            scrollTrigger: {
                trigger: "#learnSkills_cards", // La section contenant les cards
                start: "top 80%", // Démarre quand le haut de .card-container atteint 80% de la fenêtre
                end: "bottom 50%", // Fin quand le bas atteint 20%
                scrub: true, // Pas de scrub, l'animation se joue normalement
                markers: false, // Montre les marqueurs (à retirer pour la prod)
    ...
    if (skills_sentences) {
        function skills_sentencesFunction(tab_skills, skills_sentences) {
            tab_skills.forEach(tab_skill => {
                let cards = document.createElement("li");
                cards.className = "cards";
        ...
        skills_sentencesFunction(coding, skills_sentences);
        skills_sentencesFunction(design, skills_sentences);
        skills_sentencesFunction(CMS, skills_sentences);
        skills_sentencesFunction(learning_skills, learnSkills_sentences);


        gsap.set("#skills_sentences li > h4", { transformOrigin: "0 50%" })
        gsap.set("#skills_sentences li:not(:first-of-type, :last-of-type) h4", { opacity: 0.2, scale: 0.8 })

        const tl = gsap.timeline()
            .to("#skills_sentences li:not(:first-of-type, :last-of-type) h4",
